	...
	override static void UpdateItemInfoLiquidType(Widget root_widget, EntityAI item)
	{
		...
		else if( item_base && item_base.GetQuantity() > 0 && item_base.IsLiquidContainer() )
		{
			...
			switch(liquid_type)
			{
				...
				{
					WidgetTrySetText(root_widget, "ItemLiquidTypeWidget", "#STR_FS_AVIATION_FUEL", Colors.ORANGE);
					break;
				}
				case LIQUID_GASOLINE:
				{
					WidgetTrySetText(root_widget, "ItemLiquidTypeWidget", "#inv_inspect_gasoline", Colors.YELLOW);
					break;
				}
				
				case LIQUID_DIESEL:
				{
					WidgetTrySetText(root_widget, "ItemLiquidTypeWidget", "#inv_inspect_diesel", Colors.YELLOW);
					break;
				}
